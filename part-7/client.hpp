...
public:
    ...
    void run() {
        ...
        while (true) {
            // Algorithm choice
            while (true) {
                std::cout << "Choose an algorithm (MST, clique, flow, SCC) or 'exit' to quit: ";
                std::getline(std::cin, algo);
                if (algo == "exit") return;

                std::string lowerAlgo = algo;
                std::transform(lowerAlgo.begin(), lowerAlgo.end(), lowerAlgo.begin(), ::tolower);
                if (lowerAlgo == "mst" || lowerAlgo == "clique" || lowerAlgo == "flow" || lowerAlgo == "scc") {
                    algo = lowerAlgo; // normalize to lowercase for sending
                    break;
                }
                std::cout << "Invalid algorithm. Try again.\n";
            }
            ...
            // Optional seed
            while (true) {
                std::cout << "Do you want to provide a seed? (y/n): ";
                std::getline(std::cin, msgS);
                if (msgS == "exit") return;

                std::string lower = msgS;
                std::transform(lower.begin(), lower.end(), lower.begin(), ::tolower);
                if (lower == "y") { useSeed = true; break; }
                else if (lower == "n") { useSeed = false; break; }
                else std::cout << "Enter 'y' or 'n'.\n";
            }
            ...
            if (useSeed) {
                ...
                std::getline(std::cin, msgS);
                if (msgS == "exit") return;
                fullMsg = algo + " " + std::to_string(v) + " " + std::to_string(e) + " " + msgS;
            } else {
                fullMsg = algo + " " + std::to_string(v) + " " + std::to_string(e);
            }
